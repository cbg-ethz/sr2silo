...
def read_fusion(r1, r2, refseq, header):
    ...
    ), f"Different gap starts: {gs} ({r1.pos} + {r1.query_alignment_length} - {num_inserts}) vs {get_last_valid_pos(ll1) + 1}\n{ll1}"
        ...
        if (ge - gs) == -1:  # no gap in fact, ends meet
            ...
            if (ge - gs) < -1:  # when no gap, but an overlap
                ...
                print(
                    len(outr.seq),
                    len(r1.seq),
                    len(r2.seq[(len(r2.seq) - restlen - 1) :]),
                    len(outr.seq),
                )
            else:  # proper gap
                # outr.cigarstring = str(outr.template_length) + "M"  #obvious cheating  ## FIX IT !!!
                outr.cigarstring = (
                    r1.cigarstring + str(ge - gs + 1) + "M" + r2.cigarstring
                )
...
def fuse_reads(argv):
    ...
    if ooo:
        printf(
            f"Warning {ooo} reads not grouped in pairs (not sorted alphabetically by name)"
        )
    if ooo:
